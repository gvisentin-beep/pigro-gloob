(async function () {
  ...
  try {
    const response = await fetch("/api/compute_leva", { cache: "no-store" });
    ...
    if (!chartCanvas) throw new Error("Canvas principale non trovato");
    if (!ddCanvas) throw new Error("Canvas drawdown non trovato");
    ...
    if (metricsBox) {
      ...
      const years = yearsBetween(labels[0], labels[labels.length - 1]);
      ...
      const extraRendimento = isFinite(cagrPigro) && isFinite(cagrLeva)
        ? (cagrLeva - cagrPigro)
        : NaN;
      ...
      metricsBox.innerHTML = `
        <div style="margin-top:16px; line-height:1.6;">
          ...
          <div style="margin-top:10px;"><b>Valore finale Pigro:</b> ${euro(finalPigro, 0)}</div>
          <div><b>Valore finale Pigro Leva 20%:</b> ${euro(finalLeva, 0)}</div>
          <div><b>Vantaggio/Svantaggio leva:</b> ${euro(diff, 0)}</div>

          <div style="margin-top:10px;"><b style="color:#1f77b4">CAGR Pigro:</b> ${pct(cagrPigro, 2)}</div>
          <div><b style="color:#e63946">CAGR Pigro Leva:</b> ${pct(cagrLeva, 2)}</div>
          <div><b>Extra rendimento leva:</b> ${isFinite(extraRendimento) ? pct(extraRendimento, 2) : "—"}</div>
